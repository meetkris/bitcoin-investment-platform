...
export default function TrashPage() {
    ...
    async function fetchEmails() {
        try {
            setLoading(true);
            const response = await fetch("/api/webmail?folder=[Gmail]/Bin&limit=50");
    ...
    return (
        <div className="flex flex-col h-full">
            <header className="h-16 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 bg-white dark:bg-slate-900">
                <h1 className="text-xl font-bold">Trash ({emails.length})</h1>
                <button
                    onClick={fetchEmails}
                    disabled={loading}
            ...
            <div className="flex-grow overflow-auto bg-white dark:bg-slate-900">
                {loading ? (
                    ...
                    <div className="p-8 text-center text-slate-500">Trash is empty</div>
                ) : (
                    emails.map((email) => (
                        <div key={email.id} className="flex items-center gap-4 p-4 border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors opacity-60">
                            <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900 flex items-center justify-center text-red-600 dark:text-red-300 font-medium shrink-0">
                                🗑️
                            </div>
